...
export default function Dashboard() {
  ...
  const systemStatus = metrics ? Math.min(95, (metrics.cpuUsage + metrics.memoryUsage + metrics.networkStatus) / 3) : 85
  const cpuUsage = metrics?.cpuUsage || 42
  const memoryUsage = metrics?.memoryUsage || 68
  const networkStatus = metrics?.networkStatus || 92
  const securityLevel = 75 // Keep this simulated for now
  const uptime = metrics?.uptime || "0d 00:00:00"
  const hostname = metrics?.systemInfo.hostname || "SERVER-01"
  const totalMemory = metrics?.systemInfo.totalMemory || 24
  const freeMemory = metrics?.systemInfo.freeMemory || 8
  const cpuCount = metrics?.systemInfo.cpuCount || 8
  ...
  return (
    ...
    >
      ...
      {error && (
        <div className="absolute top-4 right-4 bg-red-900/80 border border-red-500/50 rounded-md p-3 z-50">
                ...
                <CardContent className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <MetricCard
                      ...
                      trend="up"
                      color="cyan"
                      detail={`${cpuCount} Cores | ${(cpuUsage * 0.1).toFixed(1)} GHz`}
                    />
                    <MetricCard
                      title="Memory"
                      value={memoryUsage}
                      ...
                      trend="stable"
                      color="purple"
                      detail={`${totalMemory - freeMemory} GB / ${totalMemory} GB`}
                    />
                    <MetricCard
                      title="Network"
                      value={networkStatus}
                      ...
                      <TabsContent value="storage" className="mt-0">
                        <div className="bg-slate-800/30 rounded-lg border border-slate-700/50 p-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {metrics?.diskUsage.map((disk, index) => (
                              <StorageItem
                                key={index}
